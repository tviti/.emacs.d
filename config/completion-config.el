...
;;
;; Custom gptel selection interfaces
;;
(defun tviti/ivy-gptel-add-file-action (x)
  "Add file X to `gptel-context'"
  (with-ivy-window
    (let ((default-directory (ivy-state-directory ivy-last)))
      (gptel-add-file (expand-file-name x default-directory)))))

(defun tviti/ivy-gptel-add-file (&optional initial-input)
  "Prompt for a file in the current Git repository and add it via
`gptel-add-file'.
Uses `ivy-read' with candidates generated by `counsel-git-cands'.
Accepts an optional INITIAL-INPUT string to pre-fill the prompt."
  (interactive)
  (counsel-require-program counsel-git-cmd)
  (let ((default-directory (counsel-locate-git-root)))
    (ivy-read "File name: " (counsel-git-cands default-directory)
              :initial-input initial-input
              :action #'tviti/ivy-gptel-add-file-action)))
